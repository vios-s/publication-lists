...
4. Preserve existing extra fields from the website file (for example `image`, `code`, `pdf`) by matching publications on normalized `title + date`.
...
- Extra metadata fields (for example `image`, `code`, `pdf`) can be maintained manually in the website repo.
