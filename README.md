...
4. Preserve existing extra fields from the website file (for example `image`, `code`, `pdf`, `display`) and `website` values by matching publications on normalized `title + date`.
...
- Extra metadata fields (for example `image`, `code`, `pdf`, `display`) and `website` values can be maintained manually in the website repo.
